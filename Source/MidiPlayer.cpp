...
bool MidiPlayer::loadMidiFile (const juce::File& file)
{
    ...
    if (stream.openedOk())
    {
        ...
        if (midiFile.readFrom (stream))
        {
            ...
            {
                DBG("MidiPlayer: Warning - Duration is zero or negative! Forcing fallback duration.");
...
double MidiPlayer::getPosition() const
{
...
void MidiPlayer::getNextAudioBlock (juce::MidiBuffer& outputBuffer, int numSamples, double sampleRate)
{
    outputBuffer.clear();
    ...
    if (outputBuffer.getNumEvents() > 0)
        DBG("MidiPlayer: Pushed " << outputBuffer.getNumEvents() << " events to buffer");

    currentPositionSeconds = endPositionSeconds;

    if (currentPositionSeconds >= durationSeconds)
    {
        DBG("MidiPlayer: Reached end of duration (" << durationSeconds << "s). Stopping playback.");
