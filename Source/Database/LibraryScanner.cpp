...
LibraryScanner::~LibraryScanner() { stopThread(2000); }
...
void LibraryScanner::run() {
  ...
  if (midFiles.isEmpty()) {
    if (onStatusChanged)
      onStatusChanged("No MIDI files found.");
    if (onScanFinished)
      onScanFinished();
    return;
  }
  ...
  if (!db.beginBulkInsert()) {
    if (onStatusChanged)
      onStatusChanged("Database lock error.");
    if (onScanFinished)
      onScanFinished();
    return;
  }
  ...
  for (int i = 0; i < total; ++i) {
    ...
    juce::String lyrPathStr = pathStr.replace("\\Song\\", "\\Lyrics\\")
                                  .replace("\\song\\", "\\lyrics\\");
    ...
    if (lyrFile.existsAsFile()) {
      ...
      if (lines.size() > 1)
        record.artist = lines[1].trim();
      if (lines.size() > 2)
        record.key = lines[2].trim();

      if (record.key.isEmpty())
        record.key = "-";
      if (lines.size() > 0)
        record.lyrics =
            lines[0].substring(0, 100); // Save a quick preview for search
    }
  ...
  if (onStatusChanged) {
    juce::MessageManager::callAsync([this]() {
      if (onStatusChanged)
        onStatusChanged("Database Rebuild Complete.");
      if (onProgressChanged)
