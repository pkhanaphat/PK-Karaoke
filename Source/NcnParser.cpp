...
SongLyrics NcnParser::parse (const juce::File& lyrFile, const juce::File& curFile, int midiResolution)
{
    SongLyrics result;
    ...
    if (curFile.existsAsFile())
    {
        ...
        for (size_t i = 0; i + 1 < size; i += 2)
        {
            ...
            double seconds = (double)tick / (midiResolution * 2.0); // Assume 120 BPM (2 bps)
    ...
    for (int i = 0; i < rawLines.size(); ++i)
    {
        ...
        if (line.isEmpty())
        {
            ...
            {
                processedCursors.insert (processedCursors.begin() + currentCursorIndex, processedCursors[currentCursorIndex]);
            }
            currentCursorIndex++;
        }
        else
        {
            ...
            // Note: juce::String::length() returns number of characters, which is correct for UTF-8 here
            // because HandyKaraoke counts TIS-620 chars (1 byte = 1 char).
...
juce::String NcnParser::convertTis620ToUtf8 (const juce::MemoryBlock& data)
{
    ...
    for (size_t i = 0; i < size; ++i)
    {
        ...
        {
            // Unmapped or control characters in 0x80-0xA0 range (usually ignored or space)
            result << " ";
        }
    }
