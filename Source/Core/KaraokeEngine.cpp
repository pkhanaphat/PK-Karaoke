...
void KaraokeEngine::processBlock(juce::AudioBuffer<float> &buffer,
  ...
  if (wasPlaying && !isPlaying) {
    DBG("KaraokeEngine: processBlock detected song finish. wasPlaying=true, "
        "isPlaying=false. Triggering checkQueue().");
...
void KaraokeEngine::checkQueue() {
  ...
  if (queueManager.popNext(next)) {
    ...
    juce::String lyrPathStr = next.path.replace("\\Song\\", "\\Lyrics\\")
                                  .replace("\\song\\", "\\lyrics\\");
    ...
    juce::String curPathStr = next.path.replace("\\Song\\", "\\Cursor\\")
                                  .replace("\\song\\", "\\cursor\\");
