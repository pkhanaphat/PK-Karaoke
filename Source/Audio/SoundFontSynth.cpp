...
void SoundFontSynth::processBlock(juce::AudioBuffer<float> &buffer,
  ...
  if (midiMessages.getNumEvents() > 0) {
    DBG("SoundFontSynth: Processing " << midiMessages.getNumEvents()
                                      << " messages, Vol=" << currentVolume);
  } else if (++logCounter % 100 == 0) {
    // Periodic heartbeat to show the synth is alive
    // DBG("SoundFontSynth: Block processed (silent), Vol=" << currentVolume <<
    ...
    if (samplesToRender > 0) {
      ...
      for (int ch = 0; ch < 16; ++ch) {
        ...
        if (mixer != nullptr) {
          ...
          // Simple solo logic check (assuming mixer handles the global solo
          // state correctly, but we can query it directly here)
          if (isMuted)
            vol = 0.0f;
        }
    ...
    if (channel >= 0 && channel < 16) {
      if (msg.isNoteOn()) {
        if (channelSynths[channel] != nullptr)
          tsf_channel_note_on(channelSynths[channel], channel,
                              msg.getNoteNumber(), msg.getFloatVelocity());
        else if (mainSynth !=
                 nullptr) // Fallback to mainSynth if channel synth is missing
          tsf_channel_note_on(mainSynth, channel, msg.getNoteNumber(),
                              msg.getFloatVelocity());
      } else if (msg.isNoteOff()) {
        if (channelSynths[channel] != nullptr)
          tsf_channel_note_off(channelSynths[channel], channel,
                               msg.getNoteNumber());
        else if (mainSynth !=
                 nullptr) // Fallback to mainSynth if channel synth is missing
        ...
        if (channelSynths[channel] != nullptr)
          tsf_channel_set_presetnumber(channelSynths[channel], channel, prog,
                                       (channel == 9));
        else if (mainSynth != nullptr) // Fallback to mainSynth
          tsf_channel_set_presetnumber(mainSynth, channel, prog,
                                       (channel == 9));
      } else if (msg.isPitchWheel()) {
        if (channelSynths[channel] != nullptr)
          tsf_channel_set_pitchwheel(channelSynths[channel], channel,
        ...
        if (channelSynths[channel] != nullptr)
          tsf_channel_midi_control(channelSynths[channel], channel,
                                   msg.getControllerNumber(),
                                   msg.getControllerValue());
        else if (mainSynth != nullptr) // Fallback to mainSynth
          tsf_channel_midi_control(mainSynth, channel,
                                   msg.getControllerNumber(),
                                   msg.getControllerValue());
      }
    } else {
      // SysEx or other global messages could be sent to all synths or just
      // mainSynth For now, we'll send to mainSynth as a fallback for unhandled
      // messages
      if (mainSynth != nullptr) {
  ...
  // (This ensures that if MIDI events were sent to mainSynth directly, they are
  // heard)
