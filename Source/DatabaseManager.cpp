...
void DatabaseManager::createTables()
{
    ...
    const char* sql = 
        ...
        // FTS5 Virtual Table
        "CREATE VIRTUAL TABLE IF NOT EXISTS songs_fts USING fts5(id, title, artist);"
        
        // Triggers to keep FTS table in sync
        "CREATE TRIGGER IF NOT EXISTS songs_ai AFTER INSERT ON songs BEGIN "
        "  INSERT INTO songs_fts(id, title, artist) VALUES (new.id, new.title, new.artist); "
        "END;"
        ...
        "  UPDATE songs_fts SET title = new.title, artist = new.artist WHERE id = old.id; "
        "END;"
...
bool DatabaseManager::beginBulkInsert()
{
    ...
    const char* insertSql = "INSERT INTO songs (id, title, artist, key, tempo, path, lyrics) VALUES (?, ?, ?, ?, ?, ?, ?)";
...
int DatabaseManager::getTotalCount()
{
    ...
    int count = 0;
    sqlite3_stmt* stmt;
    if (sqlite3_prepare_v2(db, "SELECT COUNT(*) FROM songs", -1, &stmt, 0) == SQLITE_OK)
    {
...
juce::Array<SongRecord> DatabaseManager::searchSongs (const juce::String& query, int limit)
{
    ...
    {
        sql = "SELECT id, title, artist, key, tempo, path, lyrics FROM songs LIMIT " + juce::String(limit);
    ...
    else
    {
        ...
        {
            ftsQuery << "\"" << parts[i] << "\"* ";
            if (i < parts.size() - 1) ftsQuery << "AND ";
        }
...
SongRecord DatabaseManager::readRecordFromStatement (sqlite3_stmt* stmt)
{
    ...
    if (cId) record.id = juce::String::fromUTF8 (reinterpret_cast<const char*>(cId));
    if (cTitle) record.title = juce::String::fromUTF8 (reinterpret_cast<const char*>(cTitle));
    if (cArtist) record.artist = juce::String::fromUTF8 (reinterpret_cast<const char*>(cArtist));
    if (cKey) record.key = juce::String::fromUTF8 (reinterpret_cast<const char*>(cKey));
    record.tempo = sqlite3_column_int(stmt, 4);
    if (cPath) record.path = juce::String::fromUTF8 (reinterpret_cast<const char*>(cPath));
    if (cLyrics) record.lyrics = juce::String::fromUTF8 (reinterpret_cast<const char*>(cLyrics));
