...
juce::Array<YouTubeVideoInfo> YouTubeService::search (const juce::String& query, int maxResults)
{
...
juce::Array<YouTubeVideoInfo> YouTubeService::searchViaApi (const juce::String& query, int maxResults)
{
    juce::Array<YouTubeVideoInfo> results;
    ...
    if (auto stream = url.createInputStream(juce::URL::InputStreamOptions (juce::URL::ParameterHandling::inAddress)))
    {
        ...
        if (auto* items = parsed.getProperty("items", juce::var()).getArray())
        {
            for (auto& item : *items)
            {
                ...
                // Note: YouTube Search API doesn't return duration. We'd need another API call to 'videos' endpoint.
                // Leaving empty for basic implementation.
                info.duration = "";
...
juce::Array<YouTubeVideoInfo> YouTubeService::searchViaScraping (const juce::String& query, int maxResults)
{
    juce::Array<YouTubeVideoInfo> results;
    ...
    if (auto stream = url.createInputStream(juce::URL::InputStreamOptions (juce::URL::ParameterHandling::inAddress).withExtraHeaders(headers.joinIntoString("\r\n"))))
    {
        ...
        if (startIndex != -1)
        {
            ...
            if (endIndex != -1)
            {
                ...
                // Because parsing this massive JSON with Juce's var can be slow and brittle,
                // we will extract using simple string matching for now.
                
                int safetyLimit = 50000;
                int searchPos = 0;
                while (results.size() < maxResults && --safetyLimit > 0)
                {
                    int videoIdPos = jsonStr.substring(searchPos).indexOf("\"videoId\":\"");
                    if (videoIdPos == -1) break;
                    ...
                    // Prevent duplicates
                    bool isDup = false;
                    for (auto& r : results) { if (r.id == vId) { isDup = true; break; } }
                    if (isDup || vId.length() != 11) continue;
                    
                    YouTubeVideoInfo info;
                    info.id = vId;
                    
                    // Find title near it
                    int titleStartPos = jsonStr.substring(searchPos).indexOf("\"title\":{\"runs\":[{\"text\":\"");
...
juce::Array<YouTubeVideoInfo> YouTubeService::getFallbackData()
{
    ...
    results.add ({"dQw4w9WgXcQ", "Rick Astley - Never Gonna Give You Up (Official Music Video)", "Rick Astley", "", "3:33"});
    results.add ({"kJQP7kiw5Fk", "Luis Fonsi - Despacito ft. Daddy Yankee", "Luis Fonsi", "", "4:42"});
