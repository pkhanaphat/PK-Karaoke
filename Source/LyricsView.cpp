...
void LyricsView::timerCallback()
{
    if (attachedPlayer != nullptr)
    {
        ...
        if (std::abs(newTime - currentPositionSeconds) > 0.01) // Only repaint if changed slightly
        {
...
void LyricsView::paint (juce::Graphics& g)
{
    ...
    if (currentLyrics.lines.empty())
    {
        ...
        g.drawText ("No Lyrics Loaded", getLocalBounds(), juce::Justification::centred, false);
        return;
    }
    ...
    if (absoluteCharIndex > 0) absoluteCharIndex--;
    ...
    for (int i = 0; i < (int)currentLyrics.lines.size(); ++i)
    {
        ...
        if (absoluteCharIndex >= charCounter && absoluteCharIndex <= charCounter + lineLen)
        {
            activeLine = i;
    ...
    juce::Rectangle<int> topHalf = getLocalBounds().removeFromTop (getHeight() / 2).reduced (20);
    juce::Rectangle<int> bottomHalf = getLocalBounds().removeFromBottom (getHeight() / 2).reduced (20);

    // Display two lines at a time
    int line1 = (activeLine / 2) * 2;
    int line2 = line1 + 1;

    auto drawSweptLine = [&](const juce::String& text, juce::Rectangle<int> bounds, int sweepPos, bool isActive)
    {
        ...
        if (isActive && sweepPos > 0)
        {
            ...
            g.reduceClipRegion (bounds.withX (bounds.getX() + bounds.getWidth() / 2 - fullWidth / 2)
    ...
    if (line1 < (int)currentLyrics.lines.size())
        drawSweptLine (currentLyrics.lines[line1], topHalf, (activeLine == line1) ? activeCharInLine : (activeLine > line1 ? 999 : 0), true);
    
    if (line2 < (int)currentLyrics.lines.size())
        drawSweptLine (currentLyrics.lines[line2], bottomHalf, (activeLine == line2) ? activeCharInLine : (activeLine > line2 ? 999 : 0), true);

    // Draw debugging time info
    g.setFont (juce::Font(16.0f));
    g.setColour(juce::Colours::yellow);
    g.drawText("Time: " + juce::String(currentPositionSeconds, 2) + "s", 
               getLocalBounds().removeFromBottom(30), juce::Justification::centred, false);
}
